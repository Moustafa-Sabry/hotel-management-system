...
export class RoomCategoriesService {
  constructor(@InjectModel(RoomCategory.name)private readonly categoryModel: Model<RoomCategory>,) {}
  ...
  async update(id: string,updateDto: UpdateRoomCategoryDto,) {
    ...
    const category =
      await this.categoryModel.findOneAndUpdate(
        {_id: id,isDeleted: false,},updateDto,{new: true,},);
  ...
  async remove(id: string) {
    ...
    const category =await this.categoryModel.findOneAndUpdate({_id: id, isDeleted: false},{isDeleted: true},{new: true,},);
